...
Scheduler <- R6Class(classname = "Scheduler",
                        ...
                        public = list(
                          ...
                          enumerate_tasks = function(delayed_object, dependent_uuid = NULL){
                          ...
                          update_task = function(task, old_category, new_category){
                            if(private$.verbose){
                              message(sprintf("updating %s from %s to %s", task$name, old_category, new_category))
                            }
                          ...
                          compute = function(){
                            while(!private$.delayed_object$resolved){
                              ...
                              if(private$.verbose){
                                message(sprintf("run:%d ready:%d workers:%d",nrunning,nready,private$.nworkers))
                              }
                              if((nready>0) && (nrunning<private$.nworkers)){
                                ...
                                if(length(completed)>0){
                                  newly_completed <- mget(unlist(completed),private$.task_lists[["resolved"]])
                                  lapply(newly_completed,`[[`,"value") # force value collection
                                  all_dependents <- unique(unlist(lapply(newly_completed,`[[`,"dependents")))
                        ...
                        active = list(
                          ...
                          next_ready_task = function(){
                            ...
                            if(length(ready_tasks)>0){
                              counts <- sapply(ready_tasks, closest_dependent_count, private$.task_lists[['waiting']])
