#' Function to make graphical representation of a task dependency structure
...
make_graph <- function(delayed_object, graph=NULL, level=1){
  ...
  if(!(my_address%in%names(V(graph)))){
    graph <- graph + vertex(my_address, shape="rectangle", label=node_name, level=level, sequential=delayed_object$sequential, state=delayed_object$state)
...
plot.Delayed <- function(x, ...){
  ...
  nodes$color <- c("blue","red","orange","green")[match(nodes$state, c("waiting","ready","running","resolved"))]
